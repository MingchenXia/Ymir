...
\begin{definition}
    A continous and closed map $f:X\rightarrow Y$ of topological spaces is \emph{topologically finite} if for each $y\in Y$, the set $f^{-1}(y)$ is finite. 
...
\begin{corollary}
    Let $f:X\rightarrow Y$ be a topologically finite map of topological spaces. Assume that $X$ is Hausdorff. Let $x\in X$ be $U'$  be an open neighbourhood of $x$ in $X$ such that all other points in $f^{-1}(f(x))$ are in the interior of $X\setminus U'$. Then any neighbourhood $V'$ of $f(x)$ in $Y$ contains an open neighbourhood $V$ of $y$ such that for $U:=f^{-1}(V)\cap U'$ the map $g:U\rightarrow V$ induced by $f$ is topologically finite and $g^{-1}(g(x))=\{x\}$.
\end{corollary}
\begin{proof}
...
\begin{corollary}\label{cor-pushforwardsheaffinite}
    Let $f:X\rightarrow Y$ be a topologically finite map of topological spaces. Assume that $X$ is Hausdorff. Let $\mathcal{F}$ be a sheaf of sets on $X$, $y\in f(X)$. Denote by $x_1,\ldots,x_n$ ($n\in \mathbb{Z}_{>0}$) the distinct points of the fiber $f^{-1}(y)$. Then we have a canonical bijection
    \[
