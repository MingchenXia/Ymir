...
\begin{corollary}\label{cor-dimformula}
...
\end{corollary}
\begin{proof}
    The problem is local on $X$. By \cref{PropertyComplex-thm-equidimlocusopen} in \nameref{PropertyComplex-chap-propcomplex}, up to shrinking $X$, we may assume that $X$ is equidimensional of dimension $\dim_x X$. By \cref{cor-dimformula}, 
    \[
...
\begin{proof}
    We temporarily denote the sheaf stated in theproposition by $\mathcal{O}'$. From the uniqueness in \cref{PropertyComplex-prop-normalizationcas} in \nameref{PropertyComplex-chap-propcomplex}, it suffices to show that $\mathcal{O}'_{x}$ is isomorphic to $\overline{\mathcal{O}_{X,x}}$ as $\mathcal{O}_{X,x}$-algebras for any $x\in X$.
...
\begin{proposition}\label{prop-flatmorphismbasechange}
...
\begin{proposition}\label{prop-flatmorphismdimformula}
...
\begin{proof}
    ...
    (2) $\equiv$ (3): We have shown that $\Delta_{X/Y}$ is always an immersion in \cref{ConstructionComplex-prop-diagoimm} in \nameref{ConstructionComplex-chap-constructionComplex}, so $\Delta_{X/Y}$ is a closed immersion if and only if it is closed.
\end{proof}
\begin{corollary}
