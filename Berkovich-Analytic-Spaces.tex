...
\begin{proposition}\label{prop-netparacptrefinement}
...
\begin{proof}
    ...
    We first choose an open covering $\mathcal{U}$ of $X$ by open $\sigma$-compact Hausdorff and locally compact subsets. This is possible by \cref{prop-kanalyticspacelocal}.
    \textcolor{red}{Is this possible?}
    Up to replacing $\mathcal{U}$ by a refinement, we may assume that $\mathcal{U}$ is locally finite. 
...
\end{proposition}
\textcolor{red}{Many details need to be included in this proof!}
...
\begin{proof}
    ...
    We first claim that we can represent $\varphi$ and $f$ by strong morphisms $(Y,\mathcal{B},\sigma)\rightarrow (X,\mathcal{A},\tau)$ and $(X',\mathcal{A}',\tau')\rightarrow (X,\mathcal{A},\tau)$ with $\sigma$, $\tau'$, $\tau$ locally finite. 

    We first choose a locally finite net $\tau$ consisting of $k$-affinoid domains on $X$ by \cref{prop-netparacptrefinement}. 
    Consider the net $\sigma'$ consisting of all $k$-affinoid domains on $Y$ that are mapped into some element in $\tau$. Then $\varphi$ can be represented by a strong morphism $(Y,\mathcal{B},\sigma')\rightarrow (X,\mathcal{A},\tau)$. We now refine $\sigma'$ using \cref{prop-netparacptrefinement} again to construct $\sigma$. The locally finite net $\tau'$ is constructed in the same way.

    Let
    \[
        \mathcal{S}:=\left\{ (V,U,U')\in \sigma\times\tau\times \tau':\varphi(V)\subseteq U, f(U')\subseteq U \right\}.  
    \]
    For any $\alpha=(V,U,U')\in \mathcal{S}$, we write 
    \[
        W_{\alpha}:=V\times_U U',\quad \Sigma_{\alpha}=|V|\times_{|U|} |U'|,
    \]
    and we have an obvious continuous map of topological spaces $W_{\alpha}\rightarrow \Sigma_{\alpha}$.
    Let 
    \[
        \Sigma=|Y|\times_{|X|}|X'|.
    \]
    Then we have a canonical map $\pi_{\alpha}:W_{\alpha}\rightarrow \Sigma$ for any $\alpha\in \mathcal{S}$.

    We claim that for any $\alpha,\beta\in S$, 
    \[
        W_{\alpha\beta}:=\pi_{\alpha}^{-1}(\Sigma_{\alpha}\cap \Sigma_{\beta})
    \]
    is a $k$-special domain in $W_{\alpha}$ and there is a canonical isomorphism $\nu_{\alpha\beta}:W_{\alpha\beta}\rightarrow W_{\beta\alpha}$ satisfying the cocycle condition.

    In fact, if $\alpha=(V,U,U')$ and $\beta=(\bar{U},\bar{U},\bar{U'})$, we can write 
    \[
        U\cap \bar{U}=U_1\cup \cdots  \cup U_n
    \]
    for some $n\in \mathbb{N}$ and $U_1,\ldots,U_n\in \tau$. For each $i=1,\ldots,n$, we write
    \[
        \varphi_{V/U}^{-1}(U_i)\cap  \varphi_{\bar{V}/\bar{U}}^{-1}(U_i)=\bigcup_{j=1}^{p_i}V_{ij},\quad f_{U'/U}^{-1}(U_i)\cap f^{-1}_{\bar{U'}/\bar{U}}(U_i)=\bigcup_{l=1}^{q_i}U_{il}',  
    \]
    where $p_i,q_i\in \mathbb{N}$, $V_{ij}\in \sigma$ and $U_{il}'\in \tau'$ for $j=1,\ldots,p_i$ and $l=1,\ldots,q_i$. Then
    \[
        W_{\alpha\beta}=\bigcup_{i,j,l}V_{ij}\times_{U_i} U_{il}'.  
    \]
    So $W_{\alpha\beta}$  is a $k$-special domain in $W_{\alpha}$. \textcolor{red}{We need to add a few lemmas}
    We have an obvious isomorphism $\nu_{\alpha\beta}:W_{\alpha\beta}\rightarrow W_{\beta\alpha}$ satisfying the cocycle condition. We can then glue the $W_{\alpha}$'s along the $W_{\alpha\beta}$'s by \cref{prop-glueing} to obtain a $k$-analytic space $W$. By \cref{prop-gtopsubcanonical}, $W$ represents the fiber product $Y\times_X X'$.

    \textbf{Step~1.2}. Now we only assume $X$ to be paracompact and Hausdorff. 
    
    Take open paracompact coverings $\{Y_i\}_{i\in I}$ of $Y$ and $\{X_j'\}_{j\in J}$ of $X'$. The existence of these coverings follows from \cref{prop-kanalyticspacelocal}. Similar to Step~1.1, we glue the $Y_i\times_X X_j'$'s along the open subsets $(Y_i\cap Y_k)\times_X(X_j'\cap X_l')$'s, we get a locally Hausdorff $k_H$-analytic space $Y'$. 
    Then by \cref{prop-gtopsubcanonical} again, $Y'$ represents the fiber product $Y\times_X X'$.
    %Let $\sigma'$ be the collection of sets of the form $V\times_U U'$, where $V\subseteq Y_i$ for some $i\in I$, $U\subseteq X$, $U'\subseteq X_j'$ for some $j\in J$ are all $k$-affinoid domains. Then $\sigma'$ is a net of compact subsets on $Y'$. There is an obvious $k$-affinoid atlas $\mathcal{B}'$ with the net $\sigma'$. The triple $()$ 

    \textbf{Step~1.3}. We do not assume that $X$ is paracompact. Take a covering $\{X_i\}_{i\in I}$ by open paracompact subsets. Let $Y'$ be the gluing of $\varphi^{-1}(X_i)\times_{X_i}f^{-1}(X_i)$'s along $\varphi^{-1}(X_i\cap X_j)\times_{X_i\cap X_j}f^{-1}(X_i\cap X_j)$'s. Then by \cref{prop-gtopsubcanonical} again, $Y'$ represents the fiber product $Y\times_X X'$.

    \textbf{Step~2}. We handle the general case. Let $\sigma'$ be the collection of all  $k_H$-affinoid domains of the form $V\times_U U'$, where $V\subseteq Y$, $U\subseteq X$, $U'\subseteq X'$ are $k_H$-affinoid domains with $\varphi(V)\subseteq U, f(U')\subseteq U$. Then $\sigma'$ is a net on $Y'$ and there is an obvious $k_H$-affinoid atlas on $Y'$ with teh net $\sigma'$. This defines a structure  a $k_H$-affinoid spaces on $Y'$. It is clear that $Y'$ represents the fiber product $Y\times_X X'$ in the category of $k_H\text{-}\AnaCat$.
\end{proof}
