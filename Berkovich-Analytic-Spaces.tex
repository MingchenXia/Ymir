...
\begin{definition}
    ...
    \[
        \widetilde{(X,x)}:=\mathbf{P}_{\widetilde{\mathscr{H}(x)}^H/\tilde{k}^H}\left\{ \widetilde{\chi_x}(\tilde{A}^H)\right\}.
    \]
...
\begin{lemma}
    Let $X=\Sp A$ be a $k_H$-affinoid space and $x\in X$. Let $U=\Sp B$ be an open affinoid neighbourhood of $x$ in $X$. Then the natural morphism
    \[
        \widetilde{(U,x)}\rightarrow \widetilde{(X,x)}       
    \]
    is an isomorphism in $\bir_{\tilde{k}^H}$.
\end{lemma}
\begin{proof}
    Observe that the morphism $\chi_x:B\rightarrow \mathscr{H}(x)$ is inner with respect to $A$.
\end{proof}
