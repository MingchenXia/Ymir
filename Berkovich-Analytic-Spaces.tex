...
\section{Introduction}\label{sec-introduction-Berkovichspaces}
...
\begin{proof}
    ...
    \]
    As $V,W_j\in \bar{\tau}$ for $j=1,\ldots,m$, by \cref{Topology-lma-netproperty1} in \nameref{Topology-chap-topology}, we can find $U_1,\ldots,U_n\in \bar{\tau}$ such that $V=U_1\cup\cdots\cup U_n$ and each $U_i$ is contained in some $W_j$. Since $W_j\in \sigma$ for $j=1,\ldots,m$, it follows that $U_i\in \bar{\sigma}$ for $i=1,\ldots,n$.
...
\begin{proof}
    ...
    \]
    Here we applied \cref{Affinoid-cor-fiberproductaffdomain} in \nameref{Affinoid-chap-affinoid} and \cref{Affinoid-prop-finitemodulebasechange} in \nameref{Affinoid-chap-affinoid}. So $\varphi_*\mathcal{O}_{\Sp B}\cong \widetilde{B}$.

    From this (2) trivially implies (1).

    Conversely, assume (1), let $B=H^0(\Sp A,\varphi_*\mathcal{O}_{\Sp B})$. By \cref{thm-Kiehlcoh}, $B$ is a finite $A$-module. Let $B'$ denote the ring $B$ endowed with the finite Banach $A$-algebra structure as in \cref{Affinoid-prop-Banachalgebrafiniteforget} in \nameref{Affinoid-chap-affinoid}. We need to show that the identity map $B'\rightarrow B$ is admissible. Observe that the identity map is bounded by \cref{Affinoid-prop-admitobanachbdd} in \nameref{Affinoid-chap-affinoid}. By \cref{Affinoid-prop-Weirestrassdomainequivdense} in \nameref{Affinoid-chap-affinoid}, it suffices to show that the induced map $\Sp B\rightarrow \Sp B'$ is surjective. Let $\varphi':\Sp B'\rightarrow \Sp A$ be the natural morphism of $k_H$-affinoid spaces. Then 
    \[
...
\begin{lemma}\label{lma-reductionindepgoodgerm}
    Let $X=\Sp A$, $U=\Sp B$ be a $k$-affinoid spaces and $x\in X$. Let $\iota:U\rightarrow X$ be an isomorphism of $U$ with an open neighbourhood of $x$.
...
\begin{proof}
    ...
    According to \cref{lma-reductioncov}, $\widetilde{V_{i,x}}$ for $i=1,\ldots,m$ covers $\widetilde{X_x}$.
    ...
    Lift $\widetilde{f_{ij}}$ to $f_{ij}\in \mathcal{O}_{X,x}$, it suffices to take 
    \[
...
\begin{thm}\label{thm-goodgermandreduction}\label{thm-test1}
    \textcolor{red}{This is probably problematic, we will avoid using this result!}
...
\begin{proof}
    ...
    \] 
    of $\widetilde{X_x}$ refining $\{\widetilde{Y_{i,x}}\}_{i\in I}$, where $n\in \mathbb{N}$ and $f_1,\ldots,f_n$ are homogeneous elements in $\widetilde{\mathscr{H}(x)}$. According to \cref{thm-gradedreddomainbij}, we can lift each element in $\mathcal{V}$ to a $k$-analytic domain $V_{i,x}$ in $X_x$. After shrinking $X$, we may assume that $X=V_1\cup\cdots\cup V_n$. 
