...
\begin{proof}
    ...
    \textbf{Step~1}. We reduce the theorem to verifying the following fact: Assume that $X$ is a seaprated compact $k$-analytic space and $x\in X$ with $\widetilde{X_x}$ affine. Assume that $X$ is the union of two $k$-affinoid domains $Y=\Sp B$ and $Z=\Sp C$ both containing $x$ and there is a non-zero homogeneous element $\lambda\in \widetilde{\mathscr{H}(x)}$ such that
    \[
        \widetilde{Y_x}=\widetilde{X_x}\{\lambda\},\quad   \widetilde{Z_x}=\widetilde{X_x}\{\lambda^{-1}\}.
    \]
    Then $x$ has a $k$-affinoid neighbourhood in $X$.
    ...
    \] 
    of $\widetilde{X_x}$ refining $\{\widetilde{Y_{i,x}}\}_{i\in I}$, where $n\in \mathbb{N}$ and $f_1,\ldots,f_n$ are homogeneous elements in $\widetilde{\mathscr{H}(x)}$. By \cref{thm-gradedreddomainbij}, we can lift each element in $\mathcal{V}$ to a $k$-analytic domain $V_{i,x}$ in $X_x$. By shrinking $X$, we may assume that $X=V_1\cup\cdots\cup V_n$. Now an induction on $n$ reduces the theorem of our assertion.


    We observe that we are free to shrink $X$ to $k$-analytic domains of the following form: $Y'\cup Z'$, where $Y'$ and $Z'$ are $k$-affinoid neighbourhoods of $x$ in $Y$ and $Z$ respectively.

    \textbf{Step~2}. We show that after shrinking $X$, we may assume that $Y\cap Z=\Sp A$, where
    \[
        A=B\{tf^{-1}\}=C\{t^{-1}g\}  
    \]
    for some $f\in B$ and $g\in C$ and $\rho_A(f-g)<t:=\rho(\lambda)$ and $\lambda=\widetilde{f(x)}$.

    Up to shrinking $X$, we can find $f\in B$, $g\in C$ both invertible such that $\rho_B(f)=t$, $\rho_C(g^{-1})=t$ such that $\lambda$ is the reduction of both $f(x)$ and $g(x)$. It therefore follows that 
    \[
        |(f-g)(x)|<t.  
    \] 
    After shrinking $X$, we can make sure that $\rho_A(f-g)<t$.
...
\begin{proposition}
    Let $Y_y, X_x$ be $k_H$-analytic germs. Then for any morphism $Y_y\rightarrow X_x$ in $k\text{-}\GerCat$, there is a unique continuous map $\widetilde{Y_y}^H\rightarrwo \widetilde{X_x}^H$ making the diagram commutative:
    \[
        \begin{tikzcd}
