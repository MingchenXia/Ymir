...
\begin{proof}
    ...
    The problem is local, we may assume that $(X,x)=(\Spec_{W}^{\An}\mathcal{A},x')$, where $W\subseteq \mathbb{C}^N$ is an open subset with $N=\dim_x X$ and $\mathcal{A}$ is a finite $\mathcal{O}_W$-algebra, $x'$ is the unique point of $\Spec_{W}^{\An}\mathcal{A}$ over $0\in \mathbb{C}^N$. Write $\pi:(X,x)\rightarrow (\mathbb{C}^N,0)$ for the projection. By \cref{cor-specangeqnanalyt} and \cref{Topology-lma-opennhfiberclosedmap} in \nameref{Topology-chap-topology}, we may assume that 
    . ......
