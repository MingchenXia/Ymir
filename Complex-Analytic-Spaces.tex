...
\begin{proposition}\label{prop-Cnlocalring}
    Let $n\in \mathbb{N}$, $w\in \mathbb{C}^n$, then there is a natural isomorphism $\mathcal{O}_{\mathbb{C}^n,w}\cong \mathbb{C}\langle z_1,\ldots.z_n\rangle$.
\end{proposition}
The ring on the right-hand side is defined  in 
\cref{Commutative-def-ringconvpowerseries} in the chapter Commutative Algebra.
\begin{proof}
...
\end{proposition}
Recall that complex analytic local algebras are defined in \cref{Commutative-def-complexanalylocaalg} in the chapter Commutative Algebra.
...
\begin{theorem}
    ...
    \]
\end{theorem}
The analytic tensor product here is defined \cref{Commutative-def-analytictensor} in the chapter Commutative Algebra. We have shown its existence in \cref{Commutative-thm-relanalytictensorexist} in the same chapter.
...
\begin{proof}
    ...
    We first consider the case where $Y=\mathbb{C}^0$. As our problem is local, we may assume that $X_1$ and $X_2$ are both complex model spaces. From the constructions, we easily reduce to the case where $X_1$ and $X_2$ are both domains in $\mathbb{C}^m$ and $\mathbb{C}^n$ respectively. In this case, the result is proved in \cref{Commutative-lma-Cmexistencetensor} in the chapter Commutative Algebra and \cref{prop-Cnlocalring}.
