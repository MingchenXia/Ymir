...
Let $(k,|\bullet|)$ be a complete non-Archimedean valued field and $H$ be a subgroup of $R_{>0}$ such that $|k^{\times}|\cdot\Gamma\neq \{1\}$.

\begin{definition}
    Let $A$ be a $k_H$-affinoid algebra. We define the \emph{$k_H$-graded reduction} of $A$ as the $H$-graded ring
    \[
      \tilde{A}^H:=\bigoplus_{h\in H}\left\{x\in A:\rho(x)\leq  h\right\}/\left\{x\in A:\rho(x)<  h\right\}.  
    \]
\end{definition}
