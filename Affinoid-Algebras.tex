...
\section{Introduction}\label{sec-introduction-Affinoidalgebras}
...
\end{definition}
In \cref{Banach-def-Berkspectrum} in \nameref{Banach-chap-Banach}., we defined functors $\Sp:k\text{-}\AffCat\rightarrow \TopCat$, $\Sp:\text{st-}k\text{-}\AffCat\rightarrow \TopCat$ and $\Sp:k_H\text{-}\AffCat\rightarrow \TopCat$. This motivates our notation. We will freely view $\Sp A$ as an object in these categories or as a topological space.
...
\begin{example}\label{ex-Kraffinoid}
    Let $r\in \mathbb{R}_{>0}$. We let $k_r$ denote the subring of $k[[T]]$ consisting of $f=\sum_{i=-\infty}^{\infty} a_i T^i$ satisfying $|a_i| r^i\to 0$ for $i\to \infty$ and $i\to -\infty$. Define a norm $\|\bullet\|_r$ on $k_r$ as follows:
    \[
...
\end{proof}
\begin{remark}
    The argument of \cref{Banach-prop-strictconvseriesradiusBanach} in \nameref{Banach-chap-Banach} does not work here if $r\in \sqrt{|k^{\times}|}$, as in general one can not take minimal $i,j$ so that \eqref{eq-aibjnorm} is satisfied.
\end{remark}
...
\begin{proof}
    ...
    It follows that $k_r$ does not have any non-zero proper closed ideals: if $I$ is such an ideal, $k_r/I$ is a Banach $k$-algebra. By \cref{Banach-prop-Berkospecnonempty} in \nameref{Banach-chap-Banach}., $\Sp k_r$ is non-empty. So $k_r$ has to admit bounded semi-valuation with non-trivial kernel.

    In particular, by \cref{Banach-cor:maximalidealclosedinBanachring} in \nameref{Banach-chap-Banach}., the only maximal ideal of $k_r$ is $0$. It follows that $k_r$ is a field.
...
\begin{proof}
    ...
    We may assume that $a_i\in \mathring{A}$ as $k$ is non-trivially valued. By \cref{Banach-prop-Tateunivprop} in \nameref{Banach-chap-Banach}., $\varphi$ admits a unique extension to a bounded $k$-algebra epimorphism 
    \[
        \Phi:  k\{T_1,\ldots,T_n,S_1,\ldots,S_m\}\rightarrow A
    \]
    sending $S_i$ to $a_i$. By \cref{Banach-cor-bddisadmi} in \nameref{Banach-chap-Banach}., $\Phi$ is admissible. Moreover, the homomorphism $\Phi$ is surjective by our assumption. It follows that $A$ is strictly $k$-affinoid.
\end{proof}
...
\end{proposition}
\begin{proof}
    By \cref{Banach-prop-algebrafinitenormadmi} in \nameref{Banach-chap-Banach}., we can endow $A$ with a Banach norm such that $\varphi$ is admissible. Then we can apply \cref{prop-strictlykafffinite}.
\end{proof}
...
\end{proposition} 
\begin{proof}
    By \cref{prop-Krfaithflat}, we may assume that $A=k\{t^{-1}T\}$ for some $t\in \mathbb{R}^m_{>0}$. By \cref{lma-tatestrictaff}, it suffices to take $r$ so that the linear subspace of $\mathbb{R}_{>0}/\sqrt{|k^{\times}|}$ generated by $r_1,\ldots,r_n$ contains all components of $t$. Taking $n\geq 1$, we can guarantee that $k_r$ is non-trivially valued.
\end{proof}


\begin{proposition}\label{prop-fiberaffinoidmap}
...
\begin{thm}[Weierstrass preparation theorem]\label{thm-Weierstrasspreparation}
    Let $n\in \mathbb{Z}_{>0}$ and $g\in k\{T_1,\ldots,T_{n}\}$ be $X_n$-distinguished of degree $s$. Then there is a Weierstrass polynomial $\omega\in k\{T_1,\ldots,T_{n-1}\}[T_n]$ of degree $s$ and a unit $e\in k\{T_1,\ldots,T_{n}\}$ such that 
    \[
        g=e\omega.
    \]
...
\end{proposition}
\begin{proof}
    We make induction on $n$. The case $n=0$ is trivial. Assume that $n>0$. It suffices to show that for any non-zero $g\in k\{T_1,\ldots,T_n\}$, $k\{T_1,\ldots,T_n\}/(g)$ is Noetherian. By \cref{lma-perturb}, we may assume that $g$ is $T_n$-distinguished. According to \cref{thm-Weierstrassdivision}, $k\{T_1,\ldots,T_n\}/(g)$ is a finite free $k\{T_1,\ldots,T_{n-1}\}$-module. By the inductive hypothesis and Hilbert basis theorem, $k\{T_1,\ldots,T_n\}/(g)$ is indeed Noetherian.
\end{proof}
...
\end{corollary}
\begin{proof}
    This follows from \cref{Banach-prop-fsupspecexp} in \nameref{Banach-chap-Banach} and \cref{prop-Tatealgufd}.
\end{proof}
...
\end{corollary}
\begin{proof}
    This follows from \cref{Banach-prop-morphismBanachcont} in \nameref{Banach-chap-Banach} and \cref{prop-strictnoetherian}.
\end{proof}
...
\end{proposition}
\begin{proof}
    By \cref{Banach-lma-supnormbddbyrho} in \nameref{Banach-chap-Banach}., we have
    \[
        \|f\|_1\geq |f|_{\sup}
    \]
...
\begin{proof}
    ...
    By \cref{cor-noenormalization} and \cref{Banach-prop-fsupspecexp} in \nameref{Banach-chap-Banach}., we may assume that $A=k\{T_1,\ldots,T_n\}$ for some $n\in \mathbb{N}$. The result then follows from \cref{prop-maxmod}.
\end{proof}
...
\end{corollary}
\begin{proof}
    This follows from \cref{cor-noenormalization}, \cref{Banach-prop-fsupspecexp} in \nameref{Banach-chap-Banach} and \cref{prop-maxmod}.
\end{proof}
...
\begin{proof}
    ...
    \]
    Let $f^n+t_1f^{n-1}+\cdots+t_n=0$ be the minimal equation of $f$ over $k\{T_1,\ldots,T_d\}$. Then $t_i\in (k\{T_1,\ldots,T_d\})^{\circ}$ by \cref{Banach-prop-fsupspecexp} in \nameref{Banach-chap-Banach}. An induction on $i\geq 0$ shows that
    \[
...
\begin{corollary}\label{cor-rhoequsup1}
    Assume that $k$ is non-trivially valued.
...
\begin{proof}
    ...
    As $I=A\cap (I (A\hat{\otimes} k_r))$, by \cref{Banach-cor-idealclosed} in \nameref{Banach-chap-Banach}., we see that $I$ is closed in $A\hat{\otimes} k_r$ and hence closed in $A$.
\end{proof}
...
\begin{proof}
    ...
    If $\rho(f)=0$, then $f$ lies in each maximal ideal of $A$. To see this, we may assume that $A$ is a field, then by \cref{Banach-prop-Berkospecnonempty} in \nameref{Banach-chap-Banach}., there is a bounded valuation $\|\bullet\|'$ on $A$. But then $\rho(f)=0$ implies that $\|f\|'=0$ and hence $f=0$.
    ...
    So we can assume that $\rho(f)>0$. In this case, by \cref{cor-noenormalization} and \cref{Banach-prop-fsupspecexp} in \nameref{Banach-chap-Banach}., we have $\rho(f)\in \sqrt{|k^{\times}|}$. Take $a\in k^{\times}$ and $d\in \mathbb{Z}_{>0}$ so that $\rho(f)^d=|a|$. Then $\rho(f^d/a)=1$ and hence it is powerly-bounded by \cref{prop-powerbddinstrictlyaff}. It follows that there is $C>0$ so that for $n\geq 1$, 
    \[
...
\begin{example}\label{ex-Berkovichdisk}
...
\begin{proof}
    ...
    \]
    when $|a|>r$. In this case, $1-a^{-1}T$ is invertible by \cref{Banach-prop-inversesmallelementBanachring} in \nameref{Banach-chap-Banach}., so
    \[
...
\begin{proposition}\label{prop-Berkovdiskprop}
    Assume that $k$ is non-trivially valued and algebraically closed.
...
\begin{proof}
    ...
    \]
    sending $S_i$ to $a_i$. By \cref{Banach-cor-bddisadmi} in \nameref{Banach-chap-Banach}., $\Phi$ is admissible. Moreover, the homomorphism $\Phi$ is surjective by our assumption. It follows that $A$ is $k_H$-affinoid.
...
\end{lemma}
\begin{proof}
    (1) $\implies$ (2) by \cref{cor-strictlyaffsupnorm} and \cref{cor-rhoequsup1}.
...
For any $k$-affinoid algebra $A$, we have defined the category $\BanCat_A^f$ of finite Banach $A$-modules in \cref{Banach-def-finiteBanachmodules} in \nameref{Banach-chap-Banach}. We write $\ModCat_A^f$ for the category of finite $A$-modules. 
...
\begin{proof}
    ...
    According to \cref{prop-finitemodulebasechange} and \cref{prop-Krfaithflat}, we may assume that $k$ is non-trivially valued. By \cref{thm-affnoetherian}, $N$ is a Noetherian $A$-module. It follows from \cref{Banach-cor-idealclosed} in \nameref{Banach-chap-Banach} that $\Img \varphi$ is closed in $N$ and is finite as an $A$ module. In particular, the norm induced from $N$ and from $M$ are equivalent by \cref{lma-finiteBanachmodulemapbdd}. It follows that $\varphi$ is admissible.
\end{proof}
...
\begin{example}\label{ex-Laurentdomain}
    Let $A$ be a $k_H$-affinoid domain, $n,m\in \mathbb{N}$ and $f=(f_1,\ldots,f_n)\in A^n$, $g=(g_1,\ldots,g_m)\in A^m$. Let $r=(r_1,\ldots,r_n)\in\sqrt{|k^{\times}|\cdot H}^n$ and $s=(s_1,\ldots,s_m)\in \sqrt{|k^{\times}|\cdot H}^m$. Define
    \[
    ...
    By \cref{Banach-cor-rhoequsup} in \nameref{Banach-chap-Banach}., we have
    \[
...
\begin{proof}
    ...
    Take $f_1,\ldots,f_n\in A$, $g_1,\ldots,g_m\in A$ both generating the unit ideal and $r=(r_1,\ldots,r_n)\in \sqrt{|k^{\times}|\cdot H}^n$, $s=(s_1,\ldots,s_m)\in \sqrt{|k^{\times}|\cdot H}^m$ such that
    \[
    ...
    \]
    Clearly $V\cap W\subseteq Z$. We need to prove the reverse inequality. Let $x\in Z$, so we have
    \[
...
\begin{proof}
    ...
    \]
    Let $f'_1,\ldots,f'_n,g'$ be the images of $f_1,\ldots,f_n,g$ in $C$. Write $f'=(f'_1,\ldots,f'_n)$. Then by \cref{Banach-cor-invertibleintermsofsp} in \nameref{Banach-chap-Banach}., $g'$ is a unit and 
    \[
        \Sp B=\Sp C\{r^{-1} g'^{-1}f'\}.  
    \]
\end{proof}
...
\begin{proposition}\label{prop-laurentdomainfundamental}
...
\end{definition}
Recall that $\rho(A)=\sqrt{|k^{\times}|\cdot H}\cup\{0\}$ by \cref{thm-strictaffdfnequal}, so $\tilde{f}$ is well-defined. This definition is compatible with \cref{def-gradedredu} in the sense that if we regard a  $\sqrt{|k^{\times}|\cdot H}$-graded ring as an $\mathbb{R}_{>0}$-graded ring, the two definitions give the same object.
...
\begin{proof}
    ...
    By \cref{Banach-cor-rhoequsup} in \nameref{Banach-chap-Banach}., it suffices to consider each of $\Sp A/\mathfrak{p}_i$ separately, so we may assume that $A$ is an integral domain. 
    
    By \cref{cor-noenormalization}, we can take $d\in \mathbb{N}$ and a finite injective homomorphism of $k$-algebras $\iota:k\{T_1,\ldots,T_d\}\rightarrow A$. According to \cref{Banach-prop-fsupspecexp} in \nameref{Banach-chap-Banach}., $\rho_A$ is the restriction of the norm $\|\bullet\|_{\Frac A}$ on $\Frac A$ induced by the finite extension $\Frac A/\Frac k\{T_1,\ldots,T_d\}$ from the Gauss valuation. But it is well-known that $\|\bullet\|_{\Frac A}$ is the maximum of finitely many valuations on $\Frac A$. \textcolor{red}{Reproduce BGR3.3.3.1 somewhere.} The assertion is by now obvious.
\end{proof}
...
\begin{proof}
    ...
    \[
        |f(x)|=r  
    \]
    for any $x\in \Sp A\{r^{-1}f\}$. It follows from \cref{Banach-cor-rhoequsup} in \nameref{Banach-chap-Banach} that for any $n\in \mathbb{Z}_{>0}$,
    \[
...
\begin{proof}
    ...
    By \cref{Commutative-cor-finitegradeddescent} in \nameref{Commutative-chap-commutative} and \cref{prop-finiteadmissibledesc},  we may assume that $k$ is non-trivially valued and $\rho(A)=\rho(B)=|k|$. By \cref{Commutative-lma-rhoconditionimplygraded} in  the chapter Commutative Algebra, we have $\tilde{A}=\tilde{A}_1\otimes_{\tilde{k}_1} \tilde{k}$. 
    According to \cref{cor-finitehomoadmi}, $\phi$ is automatically admissible if it is finite. 
    ...
    We will show that the conditions of \cref{Banach-lma-finitemodulejapanese} in \nameref{Banach-chap-Banach} is satisfied with $\rho_B$ as the norm $B$. We have shown that $\rho_B$ is a faithful $k\{T_1,\ldots,T_d\}$-algebra nrom in \cref{cor-rhofaithful}. As $B$ is of finite over $k\{T_1,\ldots,T_d\}$, the rank condition is clearly satisfied. It remains to establish that $\mathring{\phi}$ is integral.
...
\begin{lemma}\label{lma-tildebaseextensionfinite}
    Let $A$ be a $k_H$-affinoid algebra and $K/k$ be an analytic field extension. Then the natural homomorphism
    \begin{equation}\label{eq-naturalmapbaseextensionreductiong}
        \tilde{A}^H\otimes_{\tilde{k}^H}\tilde{K}^H\rightarrow \widetilde{A\hat{\otimes}_k K}^H  
    \end{equation}
    is finite.
\end{lemma}
\begin{proof}
    Take $n\in \mathbb{N}$, $r=(r_1,\ldots,r_n)\in \sqrt{|k^{\times}|\cdot H}^n$ and an admissible epimorphism
    $\pi:k\{r^{-1}T\}\rightarrow A$. Then the induced map
    \[
          \pi_K:K\{r^{-1}T\}\rightarrow A\hat{\otimes}_k K
    \]
    is an admissible epimorphism. By \cref{thm-reductionfinite}, its reduction
    \[
        \widetilde{\pi_K}:\tilde{K}^H[r^{-1}T]\rightarrow \widetilde{A\hat{\otimes}_k K}^H  
    \]
    is finite. It remains to show that the image of $\widetilde{\pi_K}$ is contained in the image of \eqref{eq-naturalmapbaseextensionreductiong}. 

    For this, we just observe that for $i=1,\ldots,n$, $\widetilde{\pi_K}(T_i)\neq 0$ if and only if $\rho(\pi_K(T_i))=r_i$. The latter is equivalent to that $\rho(\pi(T_i))=r_i$. In particular, $\widetilde{\pi_K}(T_i)$ is the image of $\pi(T_i)$ under \eqref{eq-naturalmapbaseextensionreductiong}. Our assertion follows.
\end{proof}

\begin{lemma}\label{lma-tildetensorproductfinite}
    Let $A$ be a $k_H$-affinoid algebra and $B$, $C$ be $k_H$-affinoid algebras over $A$. Then the natural homomorphism 
    \begin{equation}\label{eq-tildetensorproductfinite}
        \tilde{B}^H\otimes_{\tilde{A}^H}\tilde{C}^H\rightarrow   \widetilde{B\hat{\otimes}_A C}^H
    \end{equation}
    is finite.
\end{lemma}
\begin{proof}
    Take $n,m\in \mathbb{N}$, $r=(r_1,\ldots,r_n)\in \sqrt{|k^{\times}|\cdot H}^n$, $s=(s_1,\ldots,s_m)\in \sqrt{|k^{\times}|\cdot H}^m$ and admissible epimorphism
    $\pi:A\{r^{-1}T\}\rightarrow B$, $\pi':A\{s^{-1}S\}\rightarrow C$. Then we have an admissible epimorphism
    \[
        \pi\hat{\otimes}_A \pi':A\{r^{-1}T,s^{-1}S\}\rightarrow B\hat{\otimes}_A C.  
    \]
    By \cref{thm-reductionfinite}, the reduction
    \[
        \widetilde{\pi\hat{\otimes}_A \pi'}:\tilde{A}^H[r^{-1}T,s^{-1}S]\rightarrow \widetilde{B\hat{\otimes}_A C}^H
    \]
    is finite. It suffices to show that the image of this map is contained in the image of \eqref{eq-tildetensorproductfinite}. The argument is similar to that in \cref{lma-tildebaseextensionfinite}, and we omit it. \textcolor{red}{Include it}
\end{proof}
...
\begin{lemma}\label{lma-nongradedredsurj}
...
\begin{proof}
    ...
    By \cref{Banach-cor-Spalgclosure} in \nameref{Banach-chap-Banach}, we have a natural identification 
    \[
    ...
    Let $K$ be the smallest extension of $\Frac k\{T_1,\ldots,T_d\}$ inside this algebraic closure which is norm over $\Frac k\{T_1,\ldots,T_d\}$ and which contains $A$. Let $G=\Gal(K/\Frac k\{T_1,\ldots,T_d\})$. 
    Then let $B$ be the smallest $k$-subalgebra of $K$ containing all $\gamma(A)$ for $\gamma\in G$. Then $B$ is finite over $k\{T_1,\ldots,T_d\}$ and hence strictly $k$-affinoid by \cref{prop-strictlykafffinite2}.
    ...
    %Observe that the points in this fiber are necessarily maximal ideals in the tensor product as $\tilde{B}$ is finite over $\tilde{k}[T_1,\ldots,T_d]$. 
...
\begin{thm}\label{thm-gradedreductionsurjective}
...
\begin{proof}
    ...
    By \cref{Commutative-lma-rhoAequalrhok} in \nameref{Commutative-chap-commutative}, it suffices to show that the usual reduction $\pi:A\rightarrow \Spec \tilde{A}$ is surjective, which is exactly \cref{lma-nongradedredsurj}.
\end{proof}

\begin{proposition}\label{prop-genericptuniqlift}
...
\begin{proof}
    ...
    \[
        \rho_A(f)\leq |f(x)|.  
    \]
    So equality holds by \cref{Banach-cor-rhoequsup} in \nameref{Banach-chap-Banach}. In other words, $\pi^{H,-1}(\tilde{x})=\{\rho_A\}$.

    In general, by \cref{Commutative-lma-specggenericpointfinite} in \nameref{Commutative-chap-commutative}, we can find $\tilde{f}\in \tilde{A}^H$ that is not contained on all generic points of irreducible components by $x$. \textcolor{red}{Include graded version of prime avoidance somewhere.} Lift $\tilde{f}$ to $f\in A$ and $r=\rho_A(f)$. Let $B=A\{r^{-1}f\}$, then
    \[
...
\end{proposition}
Recall that the  Shilov boundary is defined in \cref{Banach-def-Shilovboundary} in \nameref{Banach-chap-Banach}.
...
\begin{definition}
    Let $A$ be a $k_H$-affinoid algebra, $M$ be an $A$-module and $\mathcal{V}=\{V_i\}_{i\in 1,\ldots,n}$ be a finite covering of $\Sp A$ by $k_H$-affinoid domains. We define the \emph{augmented \v{C}ech complex} $\check{C}(\mathcal{V},M)$ as the following cochain complex with $M$ placed at the place $0$:
    \[
...
\begin{proof}
    ...
    Take $\mathcal{V}$ as the Laurent covering generated by $(r_i r_j^{-1})^{-1} f_if_j^{-1}$ for $1\leq i<j\leq n$. We claim that $\mathcal{V}$ refines $\mathcal{U}$. 
...
\begin{proof}
    ...
    Take a $k$-free polyray $s$ with at least one component. By \cref{prop-Krfaithflat}, we can make the base change to $k_s$ and assume that $k$ is non-trivially valued. In this case, by open mapping theorem \cref{Banach-thm-openmapping} in \nameref{Banach-chap-Banach}., the admissibility is automatic. It suffices to prove the exactness.
...
\begin{corollary}\label{cor-affdomainresidueequal}
...
\begin{thm}[Tate acyclicity theorem]\label{thm-Tateacyc}
    Let $A$ be a $k$-affinoid algebra and $M$ be an $A$-module. Let $\mathcal{V}=\{V_i\}_{i\in 1,\ldots,n}$ be a finite $k$-affinoid covering of $\Sp A$.
...
\begin{definition}
    ...
    \begin{proof}
        ...
        Write $B_{ij}=B_i\hat{\otimes}_A B_j$. 
...
\begin{corollary}\label{cor-affandclosedimmdecomposition}
...
\begin{proof}
    ...
    \]
    is surjective, where $d^0(m_1,m_2)=m_1-m_2$. Note that we have omitted the obvious map $M_1\rightarrow M_{12}$ and $M_2\rightarrow M_{12}$. 
    ...
    It remains to prove the claim. We have a short exact sequence
    \[
...
\section{Boundaryless homomorphism}
Let $(k,|\bullet|)$ be a complete non-Archimedean valued field and $H$ be a subgroup of $\mathbb{R}_{>0}$ such that $|k^{\times}|\cdot H\neq \{1\}$.
...
\begin{proposition}\label{prop-innerhomochar}
...
\begin{proof}
    ...
    By \cref{Commutative-lma-gradedhomokernel} in \nameref{Commutative-chap-commutative}, we can find polynomials $G_l\in R[n^{-1}T_1,\ldots,(2n-1)^{-1}T_n]$ and $H_l\in R[T_0]$ of degree $l$ such that $\deg_{T_0}H_l\leq n-1$ and $T_0^l-G_l-H_l\in \ker \Phi$, where 
    \[
...
\begin{proposition}\label{prop-intcomposition}
...
\begin{proposition}\label{prop-affinoiddomaininterior}
